...
**EfficientCrackNet (arXiv:2409.18099).** MobileViT + Edge Extraction Module + ULSAM; mIoU 87.10 %, F1 85.88 % on DeepCrack — strong edge-deployment candidate.
...
Diffusion methods consistently improve topological continuity but **inference cost (iterative denoising) disqualifies them from real-time drone use without distillation**, and none has been validated on multi-class DACL10k-style benchmarks.
...
**SAM/SAM2 with PEFT is the high-leverage 2024–2026 frontier.** Zero-shot SAM is not competitive on thin cracks (Ahmadi 2023; Ge 2024; SAC 2025). However, LoRA/Adapter (~1 % trainable params) or LayerNorm-only tuning (SAC) brings SAM to SOTA on out-of-distribution test sets, often surpassing fully supervised CNNs. For drone video, SAM2's memory-based propagation (arXiv:2408.00714) enables one-shot key-frame prompting and ~44 FPS streaming inference; SECrackSeg and Duan et al. 2025 are the validated concrete-specific instances. EdgeSAM/MobileSAM + ConvLoRA (Crack-EdgeSAM) brings fine-tuned SAM to embedded inference.
...
**Diffusion-based segmentation (CrackDiff, CrossDiff, CrackSegDiff)** helps thin-structure continuity but is incompatible with real-time drone use unless distilled. No multi-class DACL10k-style validation exists.
...
### Practical accuracy-vs-throughput table (drone deployment)

| Family | Thin-crack accuracy | Params | Typical FPS | Drone fit |
|---|---|---|---|---|
| Improved DeepLabV3+ / HrSegNet | High (mIoU 0.73–0.83) | 5–10 M | 50–150 | ★★★ Onboard |
| SegFormer-B0/B1 | High once decoder patched | 4–14 M | 40–100 | ★★★ Good |
| Mask2Former / Swin-L | Highest multi-class accuracy | 100–220 M | 5–15 | ★ Ground-station only |
| Hybrid-Segmentor | SOTA for cracks | ~60 M | 15–25 | ★★ Needs GPU |
| CrackSAM (ViT-H + LoRA) | SOTA cross-domain | ~640 M (frozen) | 1–5 | ★ Offline |
| EdgeSAM/Crack-EdgeSAM | Near-SAM quality | ~40 M | 30–60 | ★★★ Best edge SAM |
| SAM2-tiny + key-frame propagation | High and temporally consistent | 38.9 M | ~47 FPS (A100) | ★★★ Best video |
| Diffusion (CrackDiff/CrossDiff) | Top continuity | 50–150 M | <1 | ✗ Not real-time |
| DINOv2 ViT-S/14 (frozen + head) | Excellent features | 22 M | 60+ | ★★★ Feature extractor |
...
**Flip-n-Slide** (ICLR 2024 ML4RS Workshop). Each pixel appears in 8 tiles with 0–75 % overlap + flips. **+15.8 % precision for under-represented classes** — directly relevant for under-represented DACL10k classes like Crack and Cavity.
...
**Practical recipe for 4K drone video:** 1024×1024 tiles (512×512 if GPU-bound), 12.5–25 % overlap using NIST exact-tile inference with halo equal to the network's effective receptive field, Gaussian-weighted blending at seams, Flip-n-Slide augmentation at training time. HRCSF (Chu 2024) demonstrates 4K at 90.89 % IoU and 3.84 FPS on RTX 3060 — a realistic performance target.

---

## 7. Drone, video, and real-time considerations
...
**Video segmentation and temporal consistency.** SAM2 (Ravi et al. 2024, arXiv:2408.00714) is the dominant 2024–2026 video-segmentation foundation model — promptable in the first frame, memory transformer propagates masklets at ~44 FPS, trained on SA-V. *Code: facebookresearch/sam2 (Apache-2.0).* **SSP — Semantic Similarity Propagation** (arXiv:2503.15676, 2025) is the lightweight onboard alternative: image model + pose-aware linear frame-to-frame propagation + temporally consistent knowledge distillation. **Harb et al. (2024, arXiv:2411.04620)** directly compares mono-temporal U-Net vs multi-temporal Swin UNETR on 1,356 × 32 crack sequences: multi-temporal IoU 82.72 % / F1 90.54 % vs mono-temporal IoU 76.69 % / F1 86.18 % at half the trainable parameters. **Temporal information helps crack segmentation even for near-static scenes.**

**Practical drone pipeline.** Based on the 2024–2026 evidence: YOLOv8n detector (30+ FPS edge) for box prompts → EdgeSAM or SAM2-tiny for key-frame segmentation and video propagation → post-process with skeletonization + orthogonal width quantification → laser/photogrammetry homography for metric conversion → back-project masks onto 3DGS/mesh for 3D damage map.
...
**2. Match architecture to the constraint set.** For maximum accuracy on thin DACL10k classes and if compute permits: **ConvNeXt-L or EVA-02 encoder + Mask2Former head** (the dacl-challenge-winning formula, ~0.51 mIoU). For drone-realistic compute: **HrSegNet or HRNet-W48** for onboard inference; **SegFormer with Segformer++ token-merging or CA-SegFormer / edge-feature-fusion** modifications if the user wants to keep SegFormer's pretrained weights without the 1/4-resolution penalty. The HrSegNet paper (Li 2023) is the single best empirical reference for the resolution-vs-accuracy tradeoff.

**3. Fine-tuned SAM2 + 3DGS is the emerging drone-video-to-3D-damage-map pattern.** Duan et al. 2025 (JCSHM) and Wang/Spencer 2026 (arXiv:2602.16713) are the closest analogues to the thesis pipeline. Zero-shot SAM is inadequate on cracks, but LoRA/LayerNorm fine-tuning (CrackSAM, SAC, Crack-EdgeSAM) brings SAM/SAM2 to or above supervised SOTA with ~1 % trainable parameters. SAM2's memory-based video propagation enables one-shot key-frame prompting — exactly the right primitive for drone video.
...
**5. The thesis's 2D mask resolution and output format must be matched to 3D downstream use.** Coarse or downsampled masks destroy 3D fidelity for cracks. Best-practice is native-resolution tiled inference (NIST exact-tile; Flip-n-Slide augmentation; 1024² tiles, 12.5 % overlap, Gaussian blending). The 3D pipeline should combine photogrammetry (COLMAP+MVS) for measurement-grade geometry and 3DGS (via Gaussian Grouping, LBG, or AVGS) for visualization-grade semantic digital twin.
